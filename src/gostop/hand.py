...
class CardList(object):
    ...
    def __eq__(self, other):
        if other is None:
            ...
            return set(self.cards) == set(other.cards) and len(self.cards) == len(
                other.cards
            )
...
class TakenCards(CardList):
    ...
    def score_animals(self):
        ...
        if all(bird_card in animal_cards for bird_card in [BUSH_WARBLER, CUCKOO, GEESE]):
    ...
    def score_ribbons(self):
        ...
        has_red = all(
            card in ribbon_cards for card in [WISTERIA_RED, IRIS_RED, BUSH_CLOVER_RED]
        )
